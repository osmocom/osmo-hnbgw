...
{
	struct osmo_ss7_instance *ss7 = osmo_sccp_get_ss7(cnlink->hnbgw_sccp_inst->sccp);
	LOGP(DCN, LOGL_NOTICE, "using: cs7-%u %s <-> %s %s %s\n",
	     ss7->cfg.id,
	     osmo_ss7_pointcode_print(ss7, cnlink->local_addr.pc),
...
int hnbgw_cnlink_start_or_restart(struct hnbgw_cnlink *cnlink)
{
	...
	if (cnlink->hnbgw_sccp_inst) {
		if (hnbgw_cnlink_sccp_cfg_changed(cnlink)) {
			...
			LOG_CNLINK(cnlink, DCN, LOGL_NOTICE, "SCCP config changed, restarting this CN link\n");
			hnbgw_cnlink_drop_sccp(cnlink);
		} else {
			LOG_CNLINK(cnlink, DCN, LOGL_INFO, "No SCCP config change, no need to restart this CN link\n");
...
#define LOG_NRI(LOGLEVEL, FORMAT, ARGS...) \
	...
	if (map->l3.mi.type == GSM_MI_TYPE_TMSI) {
		if (osmo_tmsi_nri_v_get(&nri_v, map->l3.mi.tmsi, nri_bitlen)) {
			LOG_NRI(LOGL_ERROR, "Unable to retrieve NRI from TMSI 0x%x, nri_bitlen == %u\n", map->l3.mi.tmsi,
				nri_bitlen);
			nri_v = -1;
		} else if (map->l3.from_other_plmn) {
	...
	llist_for_each_entry(cnlink, &cnpool->cnlinks, entry) {
		...
		if (nri_matches_cnlink) {
			if (is_null_nri) {
				LOG_NRI(LOGL_DEBUG, "matches %s %d, but this NRI is also configured as NULL-NRI\n",
