...
int context_map_set_cnlink(struct hnbgw_context_map *map, struct hnbgw_cnlink *cnlink_selected)
{
	...
	hsi = cnlink_selected->hnbgw_sccp_inst;
	if (!hsi) {
	...
	LOGP(DRUA, LOGL_NOTICE, "New conn: %s '%s' RUA-%u <-> SCCP-%u %s%s%s %s l=%s<->r=%s\n",
	     ...
	     cnlink_selected->use.remote_addr_name ? : "",
	     hsi->name,
	     osmo_ss7_pointcode_print(osmo_ss7_instance_find(hsi->cs7_instance), cnlink_selected->local_addr.pc),
	     osmo_ss7_pointcode_print2(osmo_ss7_instance_find(hsi->cs7_instance), cnlink_selected->remote_addr.pc)
	    );
